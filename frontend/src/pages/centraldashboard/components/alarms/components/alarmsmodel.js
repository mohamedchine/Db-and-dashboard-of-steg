//this is like a reusable component
...
const Alamrsmdl = ({ alarms: initialAlarms = [], title = "Alarms", subtitle = "Alarm list" }) => {
   const{alarms,deleteAlarm} = useDeleteAlarm(initialAlarms);

  const handleMaintenance = (alarm) => {
    console.log('Start maintenance for alarm:', alarm.id);
  };
            ...
            {alarms.length > 0 ? (
              alarms.map((alarm) => (
                <TableRow key={alarm.id} sx={{ '&:hover': { backgroundColor: '#fafafa' } }}>
                  ...
                  <StyledBodyCell>
                    <AlarmCodeLabel>{alarm.alarm_code}</AlarmCodeLabel>
                  </StyledBodyCell>
                  ...
                  <StyledBodyCell>
                    {/* Conditional Maintenance Button */}
                    {title === "Unresolved Alarms" && (
                      <Button
                        onClick={() => handleMaintenance(alarm)}
                        size="small"
                        sx={{
                          textTransform: 'none',
                          fontWeight: 500,
