...
const DirectionDashboard = () => {
  const { centrals } = useContext(AllcentralsContext);
  ...
  const handleApplyData = async () => {
    ...
    try {
      ...
      if (isPerformanceRoute) {
        ...
        const performanceResponse = await axs.post('/direction/getcentralsperformanceandperiode', {
        ...
        const incidentsResponse = await axs.post('/direction/defeq-alarms-maintenance/byperiode', {
          start: dateRange.from,
          end: dateRange.to
        });
